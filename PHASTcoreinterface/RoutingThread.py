...
class RoutingThread (threading.Thread):
    def __init__(self, threadID, locations, bixi_file, message_callback):
        threading.Thread.__init__(self)
        self.threadID = threadID
        self.origin = locations["origin"]
        self.destination = locations["destination"]
        self.processor = Routing.Routing(bixi_file)
    ...
    def get_routes(self, mode=None):
        # mode: either "recovery" or "setup"
        #       recovery:   destination station has filled up and an update is required
        #       setup:      the very first attempt at routing
    ...
    def msg_from_server(self,message):
